...
const ClientCell = ({ rowIndex }: { rowIndex: number }) => {
  ...
  const client = ActiveClients[rowIndex];
...
export const ActiveColumns: ColumnDef<Client>[] = [
  ...
  },
  {
    ...
    header: "Status",
    cell: ({ row }) => {
      const client = ActiveClients[row.index];
      return (
  ...
  },
  {
    ...
    header: "Start Date",
    cell: ({ row }) => {
        const date = ActiveClients[row.index].start;
        console.log(date)
      return (
  ...
  },
  {
    ...
    header: "End Date",
    cell: ({ row }) => {
        const date = ActiveClients[row.index].end;
        console.log(date)
      return (
        <p className="text-xs font-regular min-w-[100px]">
          { date.toLocaleDateString() }
    ...
    cell: () => {
      return (
        <div className="flex gap-1 opacity-0 group-hover/table-row:opacity-100 transition-opacity duration-200">
          ...
          <div className="status-badge cursor-pointer bg-gray-800 hover:bg-red-600">
